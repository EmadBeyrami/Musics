//
//  MusicCollectionViewDataSource.swift
//  MusicP
//
...
/*
 ...
 This is MusicCollectionViewDataSource responsible for data source of simple collectionViews.
 
 You can easily add new datas to array by confirming your UICollectionViewCell to MusicCollectionViewCell.
 
 */

protocol MusicCollectionViewDelegate: AnyObject {
...
extension MusicCollectionViewDelegate {
    func collection(willDisplay cellIndexPath: IndexPath, cell: UICollectionViewCell) { }
    func collection(_ collectionView: UICollectionView, didSelectItem index: IndexPath) { }
    func collection(_ collectionView: UICollectionView, didDeselectItemAt index: IndexPath) { }
    func colelction<T>(didSelectModelAt model: T) { }
...
class MusicCollectionViewDataSource<T: MusicCollectionViewCell>: NSObject, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    // MARK: - Variables
    var items: [T.CellViewModel] = []
    var selectItem: IndexPath?
    var collectionView: UICollectionView
    
    weak var delegate: MusicCollectionViewDelegate?
    
    // MARK: - Initializer
    init(items: [T.CellViewModel], collectionView: UICollectionView, delegate: MusicCollectionViewDelegate) {
        self.items = items
